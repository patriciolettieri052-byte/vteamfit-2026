 ...
 *
 * Ejecutar con:
 *   npx ts-node --project tsconfig.json scripts/create-admin-user.ts
 *
...
// Cargar variables de entorno desde .env.local (mismo patrón que ingest.ts)
...
async function createAdminUser() {
  ...
  if (plansError || !plans || plans.length === 0) {
    console.error('❌ No se pudieron obtener los planes:', plansError?.message)
    process.exit(1)
  }
